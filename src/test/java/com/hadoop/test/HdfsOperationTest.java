...
public class HdfsOperationTest {
    ...
    public void testConstructorWithThreadPoolSize() {
        HdfsOperation op = new HdfsOperation(localFs, testBaseDir.toString(), 1, 1, 20);
        assertNotNull(op);
    ...
    public void testExecuteAsyncParallel() throws Exception {
        int count = 10;
        List<CompletableFuture<OperationOutput>> futures = new java.util.ArrayList<>();
    ...
    @Test
    public void testAsyncExecutionOrdering() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            Path dirPath = new Path(testBaseDir + "/mkdir/1/dir_" + i);
            assertTrue("Directory should exist for index " + i, localFs.exists(dirPath));
        }
    }
}
